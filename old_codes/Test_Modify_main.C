...
//void  PlotHitTimeResidualsFitPosition( const string& fileName, std::string fitName = "")
{
    ...
    TH1D *h_avgcosthetanew = new TH1D("avgcosthetanew","avgcosthetanew",25,-1,1);
    ...
    //cout<<dsReader.GetEntryCount()<<endl;
    for( size_t iEntry = 0; iEntry < dsReader.GetEntryCount(); iEntry++ )
    
    //for( size_t iEntry = 0; iEntry < 10; iEntry++ )
    {
        //cout<<"Ankit"<<endl;
        ...
        vector<int> pmtID;
        vector<int> GTID;
        vector<double> timeResV;
        if (rDS.GetEVCount()==0) Total_event_triggered_0++;   
        if (rDS.GetEVCount()==1) Total_event_triggered_1++;   
        if (rDS.GetEVCount()==2) Total_event_triggered_2++;   
       // cout<< "iEntry = "<< iEntry << "\t" << rDS.GetEVCount()<<endl;
        //if (rDS.GetEVCount()!=1) continue;
        //Total_event_triggered++;
        //out << "Total_event_triggered = " << Total_event_triggered << endl;
        if (rDS.GetEVCount()!=1) continue;
        Total_event_triggered++;
       // cout << "Total_event_triggered = " << Total_event_triggered << endl;
        ...
        for( size_t iEV = 0; iEV < rDS.GetEVCount(); iEV++ )
        {
            ...
            try{
                ...
                for( size_t iPMT = 0; iPMT < calibratedPMTs.GetCount(); iPMT++ )
                {
                    ...
                    h_theta_nhit->Fill(pmtCoord.Unit().Dot(eventPosition_2.Unit()), rEV.GetNhitsCleaned());
